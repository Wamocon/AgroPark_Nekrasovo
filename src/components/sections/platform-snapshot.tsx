...
type PlatformTab = "guest" | "booking" | "ai" | "team";
...
const tabIcons = {
  ...
  ai: Bot,
  team: Gauge,
} satisfies Record<PlatformTab, typeof UsersRound>;

export function PlatformSnapshot() {
  const { language } = useLanguagePreference();
  const copy = appCopy[language].home.platform;
  const [active, setActive] = useState<PlatformTab>("guest");
  const content = copy.content[active];
            ...
            {copy.tabs.map((tab) => {
              ...
              return (
                <Button
                  ...
                  className={active === tab.id ? "bg-emerald-900" : ""}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-2xl border border-emerald-950/10 bg-white p-6 shadow-sm">
              <div className="mb-4 flex items-center gap-2">
                ...
                <h3 className="font-black">{copy.pipelineTitle}</h3>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                {copy.pipeline.map((stage, index) => (
                  <div key={stage.stage} className="rounded-lg bg-[#f6f3ea] p-4">
                    <span className={`mb-3 block h-1.5 w-10 rounded-full ${parkPipeline[index].tone}`} />
                    <div className="text-2xl font-black">{parkPipeline[index].count}</div>
            ...
            <div className="rounded-2xl border border-emerald-950/10 bg-emerald-950 p-6 text-white shadow-sm">
              <div className="mb-4 flex items-center gap-2">
                ...
                <h3 className="font-black">{copy.aiTitle}</h3>
              </div>
              <div className="space-y-3">
                {copy.automations.map((item, index) => {
            ...
            <div className="rounded-2xl border border-emerald-950/10 bg-white p-6 shadow-sm sm:col-span-2">
              <h3 className="font-black">{copy.offersTitle}</h3>
                ...
                {copy.offers.map((offer, index) => (
                  <article key={offer.title} className="rounded-lg border border-emerald-950/10 bg-[#f6f3ea] p-4">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="rounded-full bg-white px-2 py-1 text-xs font-black text-emerald-700">{parkOffers[index].score}</span>
                    </div>
