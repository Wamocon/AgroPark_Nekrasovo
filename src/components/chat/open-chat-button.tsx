"use client";

import { Bot } from "lucide-react";
import { appCopy } from "@/components/i18n/app-copy";
import { useLanguagePreference } from "@/components/i18n/use-language-preference";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";

export function OpenChatButton({ label, compact = false, className }: { label?: string; compact?: boolean; className?: string }) {
  const { language } = useLanguagePreference();
  const resolvedLabel = label ?? appCopy[language].chat.button;

  return (
    <Button
      variant="outline"
      className={cn(compact ? "gap-2" : "h-auto flex-col gap-2 py-4", className)}
      onClick={() => window.dispatchEvent(new CustomEvent("agropark:open-chat"))}
    >
      <Bot className="size-4" />
      <span>{resolvedLabel}</span>
    </Button>
  );
}
